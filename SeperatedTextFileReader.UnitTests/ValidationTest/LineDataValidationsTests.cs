...
namespace SeparatedTextFileReader.UnitTests.ValidationTest
{
     ...
     public class LineDataValidationsTests
    {
        ...
        [Theory, InlineData("2	Harmonize Lactobacillus acidophilus sourcing	2014-01-01	Dairy	Daisy Milks	NULL	NULL	Simple")]
        ...
        [Theory, InlineData("6	Black and white logo paper	2012-06-01 00:00:00.000	Office supplies	Clark Kent	4880	EUR	Simple")]
        public void Should_ReturnError_With_WrongSavingAmountFormat(string line)
        {

            var expectedResult = "Money (Savings amount) values should conform be numbers with a point as the decimal separator";
            var lineParser = new LineParser();
            var deserializeRowData = new DeserializeRowData<Procurement>();
            var headerColumnDictionay = lineParser.TabSeparatedParser(headerString);
            var rowColumnDictionary = lineParser.TabSeparatedParser(line);
        ...
        [Theory, InlineData("6	Black and white logo paper	2012-06-01 00:00:00.000	Office supplies	Clark Kent	4880.199567	EUR	VeryHigh")]
        public void Should_ReturnError_With_ComplexityDiffersFromGivenValues(string line)
        {

            var expectedResult = $"Column Complexity should only have following values: {String.Join(",", validComplexityTypes)}";
            var lineParser = new LineParser();
            var deserializeRowData = new DeserializeRowData<Procurement>();
            var headerColumnDictionay = lineParser.TabSeparatedParser(headerString);
            var rowColumnDictionary = lineParser.TabSeparatedParser(line);
